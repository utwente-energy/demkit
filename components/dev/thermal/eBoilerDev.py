...
class EBoilerDev(ThermalBufConvDev):
	def __init__(self,  name,  host):
		ThermalBufConvDev.__init__(self,  name,  host)
		self.devtype = "BufferConverter"

		self.commodities = ['ELECTRICITY', 'HEAT'] # INPUT, OUTPUT
		self.producingPowers = [0, 2000] # Output power of the source, in HEAT

		# Commodity wise cop, converted from HEAT:
		self.cop = {'ELECTRICITY': 1.0} # Pretty common CoP, each unit of electricity consumed produces 4 units of heat

		self.heatProduction = 0.0 # This variable holds the heat production internally of the device

		self.capacity = 	5000.0
		self.soc = 			3000.0
		self.initialSoC = 	3000.0

		self.lowMark = 0
		self.highMark = 0

		self.producingTemperatures = [0.0, 60.0]	# Output temperature of the source
